...
public:
	...
	*********************************************************************************************/
	void dropNode() {
		dropNode(false,false);
	}
	
	void dropNode(bool isFirst, bool isLast){
		if (isFirst && isLast) {
			delete this;
			return;
		}

		if(previousnode && !isFirst) 
...
public:
	...
	~DoubleLinkedList(){
		while(_length > 0) {
			if(first == last) {
				...
				first->dropNode(true,true);
			}
			else if(first->getNext() == last) {
				cout << first->getNext()->getValue() << endl;
				last = last->getPrevious();
				first->getNext()->dropNode(false,true);
			}
			else {
				cout << first->getNext()->getValue() << endl;
	...
	*********************************************************************************************/
	void dropFirst(){
		first = first->getNext();
		first->getPrevious()->dropNode(true,false);
	};
	...
	*********************************************************************************************/
	void dropLast(){
		last = last->getPrevious();
		last->getNext()->dropNode(false,true);
	};
...
int main(int argc, char* argv[]){
	...
	L->print();		//Done!
	delete L;
	std::cout << "Input something to quit. This is just here so the program stops and the output can be seen.\n";
	std::cin >> input;
