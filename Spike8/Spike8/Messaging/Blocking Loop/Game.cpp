...
Message* Game::Receive(Message* pMessage) {
    ...
    if(pMessage->GetContents()->front()->c_str == "ReceiveMessageTest") {
        ...
        returnToSender = new Message("Game", pMessage->GetSender(), "Message Succeeded");
    }
    else {
        returnToSender = new Message("Game", pMessage->GetSender(), "Message Failed");
    }
